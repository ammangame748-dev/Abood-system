...
function suggestionButtons(cfg) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('suggestion:approve').setLabel('تأييد').setStyle(ButtonStyle.Success).setEmoji(discordEmoji(cfg.suggestions.approveEmojiId || cfg.suggestions.approveEmoji, '✅')),
    new ButtonBuilder().setCustomId('suggestion:reject').setLabel('معارضة').setStyle(ButtonStyle.Danger).setEmoji(discordEmoji(cfg.suggestions.rejectEmojiId || cfg.suggestions.rejectEmoji, '❌')),
  );
}
...
client.on('interactionCreate', async interaction => {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (name === 'warn') {
        if (!interaction.memberPermissions?.has(PermissionFlagsBits.ModerateMembers)) return interaction.reply({ content: 'تحتاج إلى صلاحية إدارة الأعضاء.', ephemeral: true });
      ...
      if (name === 'report') return await handleReport(interaction, interaction.options.getMember('member'), interaction.options.getString('reason'));
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'report:modal') return await handleReport(interaction, interaction.guild.members.cache.get(interaction.fields.getTextInputValue('memberId')), interaction.fields.getTextInputValue('reason'));
...
async function handleReport(interaction, member, reason) {
  const cfg = guildConfig(interaction.guild.id);
  ...
  await channel.send({ embeds: [new EmbedBuilder().setTitle('بلاغ سري').addFields({ name: 'المبلغ', value: `<@${interaction.user.id}>`, inline: true }, { name: 'المبلغ عنه', value: member ? `<@${member.id}>` : 'غير معروف', inline: true }, { name: 'السبب', value: safeText(reason) }).setColor('#ed4245').setTimestamp()] });
