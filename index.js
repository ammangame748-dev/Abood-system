...
function guildConfig(guildId) {
  if (!store.guilds[guildId]) {
    store.guilds[guildId] = {
      ...
      tickets: { enabled: false, channelId: '', categoryId: '', supportRoleId: '', title: 'مركز الدعم', description: 'اختر نوع الطلب من القائمة لفتح تذكرة خاصة.', color: '#5865f2', image: '', options: [
...
app.get('/auth/callback', async (req, res) => {
  try {
    ...
    req.session.oauth = { access_token: token.access_token, expires_at: (Date.now() / 1000) + (token.expires_in || 604800) };
...
async function liveGuilds(session) {
  const oauth = session.oauth;
  if (!oauth || !oauth.access_token || (Date.now() / 1000) >= (oauth.expires_at || 0)) return null;
  try {
    const res = await fetch('https://discord.com/api/users/@me/guilds', { headers: { Authorization: `Bearer ${oauth.access_token}` } });
    if (!res.ok) return null;
    const json = await res.json();
    return Array.isArray(json) ? json : null;
  } catch (error) { console.error('Live guild fetch:', error.message); return null; }
}
app.get('/dashboard', requireLogin, async (req, res) => {
  let guilds = req.session.guilds || [];
  const live = await liveGuilds(req.session);
  if (live) { guilds = live; req.session.guilds = guilds; }
  const managed = guilds.filter(g => (BigInt(g.permissions || 0) & BigInt(0x20)) !== BigInt(0) || (BigInt(g.permissions || 0) & BigInt(0x8)) !== BigInt(0));
  const online = new Set([...client.guilds.cache.keys()]);
  const cards = managed.map(g => {
    const inBot = online.has(g.id);
    const icon = g.icon ? `https://cdn.discordapp.com/icons/${g.id}/${g.icon}.png?size=96` : '';
    const status = inBot ? `<span class="dot online"></span>البوت متصل` : `<span class="dot offline"></span>البوت غير موجود`;
    return `<div class="guild ${inBot ? 'live' : ''}"><div class="guild-main"><img class="guild-icon" src="${icon}" onerror="this.style.display='none'"><div><strong>${g.name}</strong><div class="tag">${status}</div></div></div>${inBot ? `<a class="btn" href="/dashboard/${g.id}/overview">فتح اللوحة</a>` : `<a class="btn secondary" href="${inviteUrl(g.id)}">إدخال البوت</a>`}</div>`;
  }).join('');
  res.send(layout('اختيار السيرفر', `<main class="main" style="max-width:1100px;margin:auto"><section class="hero"><div class="tag">WELCOME BACK</div><h1>اختر السيرفر الذي تريد إدارته</h1><p>تظهر هنا السيرفرات التي تملك فيها صلاحية الإدارة. القائمة تُحدّث تلقائياً عند كل دخول — إذا أضفت البوت للتو فقط اضغط ريفريش للصفحة.</p></section><section class="section"><div class="guilds">${cards || '<div class="notice">لم يتم العثور على سيرفرات قابلة للإدارة. تأكد أن صلاحية "guilds" مفعلة في نطاق OAuth.</div>'}</div></section></main>`, req));
});
